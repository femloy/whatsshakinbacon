...
if !active
{
	backdrop.alpha = approach(backdrop.alpha, 0, 0.1)
	if key_start_pressed && !_nopauseRooms
	doPause()
}
else if active && !instance_exists(obj_option) && !instance_exists(obj_feats) && buffer == 0
{
	backdrop.alpha = approach(backdrop.alpha, 0.6, 0.6 / 10)
	...
	{
		with self
			event_user(options[selected].event)
		exit
	}
}
