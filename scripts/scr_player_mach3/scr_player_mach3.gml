function scr_player_mach3()
{
	...
	var _accel = character == characters.milton ? 0.01 : 0.025
	
	if mach4mode
		_accel = character == characters.milton ? 0.05 : 0.1
	...
	&& !grounded)
	|| (scr_slope(x, y + 4)
	&& place_meeting(x + sign(hsp), y - 4, obj_solid)))
	&& !place_meeting(x + hsp, y, obj_metalblock)
	&& !place_meeting(x + hsp, y, obj_secretmetalblock)
	{
		var _climb = true
